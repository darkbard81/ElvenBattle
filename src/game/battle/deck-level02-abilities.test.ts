...
import monsterCoreData from '../../../assets/pf2e/monster_core/pf2e_monster_core.json';
...
describe('deck_level02 design and abilities', () => {
  ...
  it('uses unique PF2E level -1 through 2 cards with balanced categories and 20% combat growth', () => {
    const level01Ids = new Set(LEVEL01_DECK.cards.map((card) => card.id));
    const monsterLevels = new Map(
      monsterCoreData.map((monster) => [monster._id, monster.details.level.value]),
    );
