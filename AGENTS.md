...
- 핵심 룰은 `documents/Core_Rule_Spec_v0.1.md`를 기준으로 한다.
- 전체 개발 로드맵은 `documents/Plan.md`를 기준으로 한다.
- 현재 개발 환경 목표와 완료 조건은 `documents/Phase2_Goal.md`를 기준으로 한다.
- PvE 중심 구조, 전열/후열 전장, 지배력 시스템, 이벤트 기반 효과 처리, 결정론적 리플레이를 핵심 축으로 유지한다.
...
- `src/board/`: 전열/후열 슬롯과 전장 좌표 모델
- `src/dominance/`: 지배력 한계, 점유량, 장악 점수 계산
- `src/battle/`: 공격 선언, 피해 계산, 파괴 처리
...
- `documents/Core_Rule_Spec_v0.1.md`의 구조를 좁히거나 임의로 바꾸지 않는다.
- `GameState`, `PlayerState`, `CardDefinition`, `CardInstance`는 원본 데이터와 런타임 상태를 분리한다.
- `energy`는 카드 사용 비용, `dominance`는 전장 유지 한계와 장악 점수로 분리한다.
- 전장은 기본적으로 각 플레이어 2행 x 3열 구조를 따른다.
- 후열은 같은 열 전열에 의해 보호되는 기본 규칙을 유지한다.
...
1. 변경 전 `documents/Plan.md`, `documents/Core_Rule_Spec_v0.1.md`, 관련 Phase 목표 문서를 확인한다.
2. 기존 모듈 경계를 먼저 따른다.
3. 새 의존성은 필요한 이유가 명확할 때만 추가한다.
4. 파일을 수정한 뒤 `npm run format:check`, `npm run lint`, `npm test`를 우선 확인한다.
5. 타입 또는 번들 영향이 있으면 `npm run build`까지 실행한다.
6. UI 또는 Vite 진입점이 바뀌면 개발 서버 응답을 확인한다.
