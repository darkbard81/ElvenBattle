# Core Rule Spec v0.1.1 심층분석 — 지배력 시스템 반영

작성일: 2026-06-10

수정일: 2026-06-10

수정 목적: v0.1에서 누락된 **지배력(Dominance)** 개념을 창각의 아테리얼식 전장 장악 감각을 일반화한 독자 룰 자원으로 추가한다.
...
- 단, 창각의 아테리얼이 주는 “전장 장악/지배력 기반 카드 운용 감각”은 복제하지 않고 일반화하여 **Dominance System**으로 재설계한다.
...
| 시작 지배력 한계 | 3 | 1턴부터 저비용 유닛 1~2장을 배치할 수 있는 기준 |
| 지배력 한계 상한 | 10 | 후반 고지배력 카드와 보스전 밸런스 기준 |
...
| RESOURCE | 없음 | 모든 수동 행동 | 최대 자원 증가, 자원 회복, 지배력 한계 증가/재계산 | RESOURCE_CHANGED, DOMINANCE_CHANGED | maxResource 적용 -> resource 채움 -> dominanceLimit 적용 -> dominance 재계산 |
...
| players | HP, 패, 자원, 지배력 등 | 대부분의 액션 | 필수 | 필수 |
| dominanceConfig | 지배력 증가/상한/초과 정책 | 룰/시나리오 로드 시 | 필수 | 선택 |
...
interface DominanceConfig {
  ...
  cap: number;
  overloadPolicy: 'BLOCK_NEW_SUMMON_ONLY' | 'SACRIFICE_AT_END' | 'ALLOW_TEMPORARY';
}
```
...
| resource | 카드 사용 비용 지불 | 높음 |
| dominance | 전장 유지 한계, 소환 조건, 효과 조건, AI 평가 | 높음 |
| flags | 시나리오/임시 상태 | 중간 |
...
- 지배력은 카드 이미지에 표시되는 숫자이면서 동시에 소환/유지/효과 조건에 쓰이는 룰 데이터다. 따라서 `visual`이 아니라 `CardDefinition`의 정규 필드로 관리한다.

### 지배력 관련 카드 필드

| 필드 | 의미 | 예시 |
|---|---|---|
| `dominanceCost` | 이 카드를 전장에 배치하거나 유지하는 데 필요한 지배력 점유량 | 강한 유닛 3, 약한 유닛 1 |
| `dominanceValue` | 이 카드가 전장에 있을 때 제공하는 장악 점수 | 지휘관 유닛 +2, 일반 유닛 +1 |
| `dominanceRequirement` | 이 카드 또는 능력을 사용하기 위해 필요한 최소 지배력 조건 | 내 `boardValue >= 5`일 때 발동 |

v0.1.1 기본 정책은 `dominanceCost`를 전장 유지 용량으로, `dominanceValue`를 효과 조건/AI 평가/시나리오 목표용 장악 점수로 사용한다. 둘은 같은 값일 수 있지만 반드시 같을 필요는 없다.
...
|---|---|---|---|---|---|---|
| PlayCardAction | hand instanceId, targets | MAIN/COMBAT 허용, 비용 및 지배력 조건 가능 | 손패 아님, 비용 부족, 지배력 부족, 타겟 불가 | CARD_PLAYED | 비용 차감, 카드 이동/효과 등록 | action + public payload |
| SummonUnitAction | hand instanceId, slotId | MAIN, 빈 슬롯, UNIT, 지배력 한계 충족 | 슬롯 점유, 지배력 부족, 제한 위반 | UNIT_SUMMONED | 손패 -> 전장, 소환멀미 설정 | action + slot |
...
6. 소환/유지/효과 발동에 필요한 지배력 조건을 확인한다.
7. 타겟 셀렉터와 보호 규칙을 검증한다.
8. 슬롯 점유와 row restriction을 검증한다.
9. 공격 가능 상태, exhausted, summonedThisTurn, 상태 효과를 확인한다.
10. once per turn 또는 once per game 제한을 확인한다.
11. 효과 발동 조건을 평가한다.
...
v0.1.1은 두 종류의 핵심 수치를 사용한다.

| 구분 | 이름 | 성격 | 주요 용도 |
|---|---|---|---|
| 사용 자원 | `energy` | 매 턴 회복되고 카드 사용 시 소비됨 | 카드 사용 비용, 즉발 행동 비용 |
| 전장 자원 | `dominance` | 전장 유지 한계와 장악 점수를 나타냄 | 유닛 소환/유지 제한, 강력한 스킬 조건, PvE 목표, AI 평가 |

### 10.1 Energy

`energy`는 카드를 사용할 때 소비하는 일반 자원이다. v0.1의 단일 자원 모델을 유지한다.
...
`dominance`는 창각의 아테리얼식 전장 장악 감각을 일반화한 독자 룰 자원이다. 원작의 명칭이나 세부 계산식을 복제하지 않고, 디지털 TCG 엔진에서 사용 가능한 “전장 유지 용량 + 장악 점수” 모델로 재정의한다.

| 항목 | 규칙 |
|---|---|
| 기본 의미 | 플레이어가 전장에 유지할 수 있는 카드 규모와 전장 장악도를 나타내는 수치 |
| 시작값 | `limit = 3` |
| 증가 | 자기 턴 RESOURCE 페이즈에 `limit += 1`, cap 10 |
| 임시 증가 | 카드/시나리오 효과로 `temporaryLimit` 증가 가능, END에 만료 |
| 점유량 | 전장에 있는 내 카드들의 `dominanceCost` 합계를 `used`로 계산 |
| 장악 점수 | 전장에 있는 내 카드들의 `dominanceValue` 합계를 `boardValue`로 계산 |
| 소환 조건 | `used + card.dominanceCost <= limit + temporaryLimit`여야 소환 가능 |
| 효과 조건 | 카드나 능력의 `dominanceRequirement`를 만족해야 발동 가능 |
| 부족 처리 | `ERR_INSUFFICIENT_DOMINANCE` 또는 `ERR_DOMINANCE_REQUIREMENT_NOT_MET` |
| 재계산 시점 | 소환, 이동, 파괴, 컨트롤 변경, 지속 효과 변경, RESOURCE 페이즈 |
| AI 가치 | 전장 유지 여유, 장악 점수, 같은 열 점유율, 보스 목표 진행도로 평가 |

### 10.3 Dominance와 Energy의 차이

| 비교 | Energy | Dominance |
|---|---|---|
| 소비 여부 | 카드 사용 시 즉시 소비 | 기본적으로 소비되지 않고 전장 카드가 점유 |
| 회복 방식 | 매 턴 현재치가 회복 | 한계치가 성장하고 전장 변화에 따라 재계산 |
| 주 역할 | “이번 턴 무엇을 낼 수 있는가” | “전장에 어느 정도 규모를 유지할 수 있는가” |
| 카드 표시 | 코스트 아이콘 | 지배력 아이콘 |
| 밸런스 역할 | 턴별 템포 조절 | 전장 과밀, 강력 유닛 남발, PvE 목표 제어 |

### 10.4 지배력 초과 정책

v0.1.1 기본 정책은 `BLOCK_NEW_SUMMON_ONLY`다.

- 새 카드를 소환할 때는 지배력 한계를 초과할 수 없다.
- 효과로 한계가 감소하여 `used > limit + temporaryLimit`가 되면 `overloaded = true`가 된다.
- `overloaded` 상태에서는 새 소환과 일부 강력 효과 발동이 금지된다.
- MVP에서는 이미 전장에 있는 카드를 강제로 제거하지 않는다.
- v0.2에서 `SACRIFICE_AT_END` 정책을 선택하면 종료 페이즈에 초과분만큼 유닛을 선택해 묘지로 보낼 수 있다.

### 10.5 지배력 관련 이벤트

- `DOMINANCE_CHANGED`: 한계, 점유량, 장악 점수가 변경될 때 발생한다.
- `DOMINANCE_OVERLOADED`: `used > limit + temporaryLimit`가 된 순간 발생한다.
- `DOMINANCE_REQUIREMENT_MET`: 시나리오 목표나 효과 조건 달성 시 선택적으로 발생한다.

대안:

- 지배력을 단순 카드 표시 수치로만 사용: 구현은 쉽지만 전략성이 약하다.
- 지배력을 소모형 자원으로 사용: 원작 느낌과 다를 수 있고 energy와 역할이 겹친다.
- 열/행 단위 지배력으로 세분화: 위치 전략은 깊어지지만 MVP 구현 난도가 높다. v0.2 이후 확장 후보로 둔다.
...
21. 전투 결과로 유닛이 파괴되거나 컨트롤이 변경되면 양측 `dominance.used`와 `dominance.boardValue`를 재계산하고 `DOMINANCE_CHANGED` 이벤트를 기록한다.

지배력은 기본 공격 피해를 직접 증가시키지 않는다. 공격력/HP와 분리된 전장 장악 수치로 유지해야 카드 밸런스 축이 겹치지 않는다. 단, 특정 카드 효과는 `boardValue`를 참조해 피해량이나 대상 범위를 확장할 수 있다.
...
| DOMINANCE_CHANGED | `{playerId, before, after, reason}` | 소환/파괴/효과/페이즈 처리 후 지배력 재계산 | 지배력 조건부 효과 | 필수 |
| DOMINANCE_OVERLOADED | `{playerId, used, limit}` | 지배력 점유량이 한계를 초과 | 초과 페널티/시나리오 효과 | 필수 |
...
| 지배력 한계 증가 | 합산 | 효과별 | source 이탈, 만료 | 지배력 UI 강조 |
| 지배력 비용 감소 | 효과별 | 효과별 | source 이탈, 만료 | 카드/유닛 표시 |
...
type WinCondition =
  ...
  | { type: 'DOMINANCE_OBJECTIVE'; threshold: number; turns: number }
  | { type: 'SURRENDER' }
...
| 지배력 목표 | 일정 턴 동안 `boardValue`가 기준 이상이면 승리 또는 보상 | 장악전/거점전 |
...
- 지배력은 저장값을 임의 수정하지 않고 전장 카드와 지속 효과에서 재계산 가능해야 한다. 리플레이 검증 시 `dominance.used`, `dominance.boardValue`, `dominance.limit`를 state hash에 포함한다.
...
| 시작 지배력 한계 | 3 | 높음. 초반 전장 전개 속도와 과밀 방지 |
| 지배력 한계 cap | 10 | 높음. 후반 고지배력 카드 투입 가능 시점 |
| 평균 지배력 비용 | 1~3 | 높음. 전장 유지 규모와 강력 유닛 비율 |
| 평균 지배력 제공값 | 0~2 | 중간. 조건부 효과와 장악 목표 속도 |
...
| 6-1 | 지배력 시스템 | 창각의 아테리얼식 전장 장악 감각의 핵심 축 | 소환 검증, 효과 조건, AI 평가 |
...
MVP 정의: 효과 없는 기본 유닛 카드만으로 게임 시작, 드로우, 유닛 소환, 지배력 검증, 전열/후열 배치, 공격, 피해 계산, 유닛 파괴, HP 승리 판정, 리플레이 재생이 가능해야 한다.
...
| 지배력 시작값 | 초반 전장 규모 | 초반 소환 제한 불명 | limit 3 | 2 또는 4 |
| 지배력 증가 방식 | 장기 전개 속도 | 고지배력 카드 타이밍 불명 | 턴당 limit +1, cap 10 | 특정 카드/거점으로 증가 |
| 지배력 초과 처리 | 디버프/효과 충돌 | 전장 상태 불안정 | 새 소환만 금지 | 종료 페이즈 희생 |
| 지배력 카드 표시 | 카드 에셋/UX | 프레임 재작업 발생 | cost와 별도 아이콘 | 카드 타입별 숨김 |
...
Core Rule Spec v0.1은 2행 x 3열 전장을 사용하는 PvE 중심 디지털 TCG 룰이다. 각 턴은 `TURN_START -> DRAW -> RESOURCE -> MAIN -> COMBAT -> END` 순서로 진행된다. 플레이어는 `energy`를 사용해 카드를 내고, `dominance` 한계 안에서 유닛을 전장에 유지한다. 유닛은 전열/후열 슬롯에 배치되며, 같은 열 우선 공격 규칙으로 상대 유닛 또는 HP를 공격한다. 모든 행동은 `applyAction(state, action)`에서 검증 후 순수 함수에 가깝게 처리된다. 효과는 이벤트 기반으로 감지하고, 지속 효과는 레이어로 재계산한다. 리플레이는 `rngSeed`, `actionLog`, `eventLog`, 버전 정보, 스냅샷 해시로 결정론을 검증한다.
...
6. 카드 에셋 파이프라인에서 `cost`, `dominance`, `attack`, `hp`, `skillText`, `cardId` 표시 영역을 고정한다.
7. AI 평가 함수 v0.1의 가중치를 실험용 설정 파일로 분리한다.
8. 보스전 `ScenarioState`와 특수 목표 DSL을 별도 문서로 확장한다.


## 22. 카드 에셋 파이프라인 반영

지배력은 카드의 핵심 룰 수치이므로 카드 에셋에도 독립 표시 영역을 가져야 한다. 단, WebP에 합성된 텍스트와 수치는 표시용이며, 실제 룰 처리는 항상 `CardDefinition`의 원본 데이터를 기준으로 한다.

### 카드 비주얼 필수 표시값

| 표시값 | 데이터 원본 | 용도 |
|---|---|---|
| cardId | `CardDefinition.cardId` | 버전 관리, 디버그, 에셋 매칭 |
| 이름 | `nameKey`의 현지화 결과 | 카드 식별 |
| 코스트 | `cost` | 사용 자원 표시 |
| 지배력 | `dominanceCost` 또는 카드 타입별 `dominanceValue` | 소환/유지 규모와 장악 감각 표시 |
| 공격력 | `baseAttack` | 전투력 표시 |
| HP | `baseHealth` | 생존력 표시 |
| 스킬 텍스트 | `abilities`/`effectScript`의 human-readable text | 효과 이해 |
| 카드 타입/희귀도 | `type`, `rarity` | 프레임 선택 |
...
generated/
  cards/
    unit_basic_vanguard.webp
```

### 관리 원칙

- `CardDefinition`이 변경되면 해당 카드 WebP를 재생성한다.
- 에셋 파일명은 `cardId` 기준으로 생성한다.
- 카드 데이터 버전과 에셋 생성 버전을 함께 기록한다.
- 지배력 아이콘과 코스트 아이콘은 명확히 구분한다.
- 긴 스킬 텍스트는 자동 줄바꿈/폰트 축소/키워드 치환 규칙을 둔다.
- UI는 가능하면 완성된 WebP를 사용하되, 툴팁과 접근성 텍스트는 원본 카드 데이터를 사용한다.
